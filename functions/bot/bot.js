...
async function generatePrediction(pair, time) {
  try {
    ...
    const interval = intervalMap[time] || "1m";
    const symbol = (base + quote).replace(/[^A-Za-z]/g, "").toUpperCase();
    const url = `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=${interval}&limit=10`;
...
<b>𝗣𝗥𝗘𝗗𝗜𝗖𝗧𝗜𝗢𝗡:</b> <b>${trend}</b> ${trendIcon}
<b>𝗣𝗔𝗜𝗥:</b> ${pair}
<b>𝗧𝗜𝗠𝗘𝗙𝗥𝗔𝗠𝗘:</b> ${time}
