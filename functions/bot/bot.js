...
// Load verified IDs from file const loadVerifiedIds = () => { try { const data = fs.readFileSync(verifiedPath); return JSON.parse(data); } catch { fs.writeFileSync(verifiedPath, JSON.stringify([])); return []; } };

const saveVerifiedIds = (ids) => { fs.writeFileSync(verifiedPath, JSON.stringify(ids)); };

let userState = {};

// /start command bot.start((ctx) => { if (ctx.chat.id === ADMIN_ID) { return ctx.reply("🔧 Admin Commands:\n/adduser <id>\n/deleteuser <id>\n/userlist\n/stats"); } else { return ctx.reply( `👋🤖 Hi, are you ready to get a unique trading robot based on OpenAI in conjunction with 30 indicators?
...
bot.action("how_it_works", async (ctx) => { await ctx.answerCbQuery(); return ctx.replyWithHTML( `<b>Trade Mind Ai: innovative neural network-based product</b>
...
From user: @${ctx.from.username || "(no username)"} (ID: ${chatId}), { reply_markup: { inline_keyboard: [[ { text: "✅ Approve", callback_data: approve_${message}${chatId}}, { text: "❌ Reject", callback_data:reject${chatId}` }, ]], }, } ); } });
