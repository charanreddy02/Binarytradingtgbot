...
bot.action(/approve_(\d+)_(\d+)/, async (ctx) => {
  ...
  await bot.telegram.sendMessage(userId, "🎉 Your ID has been successfully verified and full access is now granted! Welcome to Trade Mind AI 🚀✅");
  await ctx.answerCbQuery("User approved.");
});

bot.action(/reject_(\d+)/, async (ctx) => {
  const userId = ctx.match[1];
  await bot.telegram.sendMessage(userId, "❌ Your ID was rejected. Please contact support.");
