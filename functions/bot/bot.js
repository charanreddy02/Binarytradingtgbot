...
After registration, click 'Enter ID' to submit your Quotex ID.`, { reply_markup: { inline_keyboard: [ [{ text: "✍️ Registration", url: "https://broker-qx.pro/sign-up/?lid=1349529" }], [{ text: "🔍 Enter Id", callback_data: "enter_id" }], [{ text: "Back", callback_data: "main_menu" }] ] }, parse_mode: "HTML" } ); });
...
bot.action(/approve_(\d+)_(\d+)/, async (ctx) => { const id = ctx.match[1]; const userId = ctx.match[2]; const ids = loadVerifiedIds(); if (!ids.includes(id)) { ids.push(id); saveVerifiedIds(ids); } await bot.telegram.sendMessage(userId, "🎉 Your ID has been approved! Full access granted. ✅"); await ctx.answerCbQuery("User approved."); });
